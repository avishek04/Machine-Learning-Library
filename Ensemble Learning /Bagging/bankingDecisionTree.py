...
def DecisionDriver():
    ...
    for i in range(T):
        ...
        for line in arr:
            ...
            for node in treeList:
                sum += 1 if GetPrediction(node, line) == "yes" else -1
    ...
    # return ID3DecisionEntropyTree(arr, np.array(["age", "job", "marital", "education", "default", "balance", "housing", "loan", "contact", "day", "month", "duration", "campaign", "pdays", "previous", "poutcome", "y"]), 0, maxDepth)

train_errors, test_errors = DecisionDriver()
plt.figure(figsize=(12, 5))

plt.plot(range(1, 500 + 1), train_errors, label='Training Error')
