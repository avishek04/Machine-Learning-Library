...
def setContiniousValueTest(testArr):
    for line in testArr:
        if (int(line[0]) < 100000):
            line[0] = "low"
...
def DecisionDriver(dataList):
    random.shuffle(dataList)
    dataArr = np.array(dataList[:24000])
    dataTestArr = np.array(dataList[24000:])
    ...
    node = ID3DecisionEntropyTree(dataArr, np.array(["balance", "sex", "education", "marriage", "age", "pay1", "pay2", "pay3", "pay4", "pay5", "pay6", "billAmt1", "billAmt2", "billAmt3", "billAmt4", "billAmt5", "billAmt6", "payAmt1", "payAmt2", "payAmt3", "payAmt4", "payAmt5", "payAmt6", "y"]), 0, maxDepth)
    
    lastInd = len(dataTestArr[0]) - 1

    errorCount = 0
    for line in dataArr:
        if line[lastInd] != GetPrediction(node, line):
            errorCount += 1

    errorTestCount = 0
    for line in dataTestArr:
        if line[lastInd] != GetPrediction(node, line):
            errorTestCount += 1

    return errorCount/24000, errorTestCount/6000

arr = setContiniousValue()    
T = 500
trainErrorList = []
testErrorList = []
for i in range(T):
    trainError, testError = DecisionDriver(arr)
    trainErrorList.append(trainError)
    testErrorList.append(testError)

plt.figure(figsize=(12, 5))
plt.plot(range(1, 500 + 1), trainErrorList, label='Test Error')
plt.plot(range(1, 500 + 1), testErrorList, label='Test Error')
plt.xlabel('Number of Iterations (T)')
plt.ylabel('Error')
plt.title('Test Errors vs. Iterations')
plt.legend()
plt.tight_layout()
plt.show()
